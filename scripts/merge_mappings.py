...
def merge_mappings(hf_mapping_file: str, local_mapping_file: str, output_file: str):
    ...
    if Path(hf_mapping_file).exists():
        with open(hf_mapping_file, 'r') as f:
            hf_mapping = json.load(f)
        logger.info(f"  Loaded {len(hf_mapping)} HuggingFace paths")
    else:
        logger.warning(f"  HuggingFace mapping not found, starting with empty mapping")
        hf_mapping = {}
    ...
    if Path(local_mapping_file).exists():
        ...
        logger.warning(f"  Local mapping not found, using only HuggingFace mapping")
        local_mapping = {}
    ...
    logger.info(f"Overlapping paths: {len(hf_mapping) + len(local_mapping) - len(complete_mapping)}")
    logger.info("=" * 60)
