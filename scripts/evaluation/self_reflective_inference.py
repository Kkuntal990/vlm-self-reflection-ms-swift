...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "and grounded answers based on the image and the user's question."
)
...
def generate_self_reflective_dialogue(
    ...
    for turn_idx in range(num_turns):
        if turn_idx == 0:
            ...
            # Generate feedback with [FEEDBACK] prompt
            feedback_prompt = FEEDBACK_PROMPT_TEMPLATE.format(
                question=clean_question, answer=prev_answer
            )

            messages_history.append({"role": "user", "content": feedback_prompt})
            ...
            # Generate refined answer with [REFINE] prompt
            refine_prompt = REFINE_PROMPT_TEMPLATE.format(
                question=clean_question, feedback=feedback
            )
